	...
	\begin{columns}[t]
		\begin{column}{.33\linewidth}
			\begin{block}{}
				...
				{\footnotesize\raggedright Har observasjonspar $(x_1,y_1),\ldots,(x_n,y_n)$.\\}
				\tema{Regresjonsmodellen}
				{\matte{&Y_i=\beta_0+\beta_1x_i+\epsilon_i, &i=1,\ldots,n}
					Forutsetningene er at
					\matte{&E(\epsilon_i)=0,&Var(\epsilon_i)=\sigma^2,& &i=1,\ldots,n}
					og $\epsilon_1,\ldots,\epsilon_n$ er inbyrdes uavhengige. Dette medfører at
					\matte{&E(Y_i)=\mu_{Y|x_i}=\beta_0+\beta_1x_i,&Var(Y_i)=\sigma_{Y|x_i}^2=\sigma^2,& &i=1,\ldots,n}
				...
				{Bruker miste kvadrat-metoden til å estimere $\beta_0$ og $\beta_1$ fra data for å få ei estimert (tilpassa) regresjonslinje for forventningslinja $\beta_0+\beta_1x$:
					\matte{\hat y=b_0+b_1x}
					MKM baserer seg på å minimere
					\matte{SSE=\sum_{i=1}^n(y_i-\hat y_i)^2=\sum_{i=1}^n(y_i-b_0-b_1x_i)^2}
					Gir estimatorer
					\matte{B_1=\frac{\sum_{i=1}^n(x_i-\bar x)(Y_i-\overline{Y})}{\sum_{i=1}^n(x_i-\overline{x})^2}=\frac{\sum_{i=1}^n(x_i-\bar x)Y_i}{\sum_{i=1}^n(x_i-\overline{x})^2}, &B_0=\overline{Y}-B_1\overline{x}}
					der
					\matte{&E(B_1)=\beta_1, &Var(B_1)=\frac{\sigma^2}{\sum_{i=1}^n(x_i-\overline x)^2}\\
					...
					\matte{S^2=\frac{SSE}{n-2}=\frac{\sum_{i=1}^n(Y_i-\hat Y_i)^2}{n-2}=\frac{\sum_{i=1}^n(Y_i-B_0-B_1x_i)^2}{n-2}}}
